...
if [ $? -eq 0 ]; then
    dialog --title "Set up instructions" --msgbox "Follow the setup instructions to configure the system." 15 60
    dialog --title "Nextauth secret" --msgbox "NEXTAUTH_SECRET should look like '^7yTjn@G$j@KtLh9&@UdMpdfDZ'" 15 60
    dialog --title "Nextauth URL" --msgbox "NEXTAUTH_URL should look like 'http://localhost:3000/api/v1/auth' this can also be a FQDN or IP if FQDN then https:// and no ports" 5 60
    dialog --title "Postgres password" --msgbox "POSTGRES_PASSWORD should be set to a strong password" 15 60
else
    dialog --title "Skipping setup instructions" --msgbox "Skipping setup instructions." 15 60
fi
